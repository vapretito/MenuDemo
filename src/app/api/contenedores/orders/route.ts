...
type MarketplaceOrderItemInput = {
  itemId: string;
  quantity: number;
};
...
export async function POST(request: Request) {
  try {
    ...
    if (!restaurant.localOrderingEnabled) {
      return NextResponse.json(
        { error: "Este restaurante todavia no acepta pedidos desde contenedores." },
        { status: 403 }
      );
    }

    if (restaurant.serviceMode === "TABLE_SERVICE") {
      return NextResponse.json(
        {
          error:
            "Este restaurante necesita pedidos iniciados desde QR de mesa. Cambialo a retiro/caja para usarlo en contenedores.",
        },
        { status: 400 }
      );
    }
    ...
    console.error("[Marketplace Order Create Error]", error);
